...
<h3 className=" whitespace-nowrap text-blue font-bold text-center text-[22px] capitalize">Search result for &quot;{searchQuery}&quot; </h3>
      ...
      {filteredProducts.length === 0 ? (
        ...
        <article className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 place-items-center gap-2 ">
          {filteredProducts.map((product) => {
              const isImageAvailable = product?.images[0]?.url && product?.images[0]?.url.trim() !== "";
            return (
              <>
              {isImageAvailable && (
                <div
